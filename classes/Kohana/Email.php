	...
	public function config_load($name='email') 
	{
		...
		if($this->_config['driver']=='smtp')
		{
			...
			'debug_output' => function($str, $level) {
				Log::instance()->add(Log::DEBUG, 'Email(:level): :message',array(':level'=>$level,':message'=>$str));
				},
			);
		}
		...
		if($driver === 'smtp')
		{
			...
			$this->_mailer->SMTPOptions = array('ssl' => array (
					'verify_peer' => false,
		...
		if($from)
		{
			...
			$from_arrgs = is_array($from) ? (Arr::is_assoc($from) ? array($from):$from) : array($from);
			call_user_func_array(array($this,'from'), $from_arrgs);
		}
